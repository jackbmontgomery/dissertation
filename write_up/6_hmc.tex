...
Each sample that is generated by HMC, or RWMH, are correlated. The
next proposal depends on where the chain is now. We know that
correlated samples carry less information than independent samples.
It is therefore crucial to track not only the number of samples that
a chain generated, but how much information is contained in these
samples. To measure this we employ effective sample size
\cite{gelman2013bayesian, geyer2011introduction}:
...
% TODO: Caption must mention the normalisation of the number of
% samples for the computational time
\begin{figure}
  \begin{center}
    \includegraphics[width=0.95\textwidth]{figures/6-ess.png}
  \end{center}
  \caption{}\label{fig:electron-ess}
\end{figure}

We plot the progress of ESS over the sampling duration of the two
algorithms in \cref{fig:electron-ess}. We can see that HMC
outperforms RWMH across each parameter.
