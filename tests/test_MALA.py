...
def main():
    ...
    mala = blackjax.mala(logdensity_fn, 1e-5)
    mala_kernel = jit(mala.step)
    init_state = mala.init(init_params, init_key)
    start_time = perf_counter()
    state, infos = inference_loop(sampling_key, mala_kernel, init_state, 100)
