...
class EqualDiffusionReactionSamplingExperiment(AbstractSamplingExperiment):
    @property
    def true_parameters(self) -> EqualDiffusionReactionParams:
    ...
    def run(
        ...
        fdm_solver = EqualDiffusionReactionFDSolver(voltammetry)
        ...
        def logdensity_fn(params: EqualDiffusionReactionParams, samples=samples):
        ...
        data_file = f"D_{sampling_algorithm}_{noise:.2f}_{sigma:.0f}.npz"
        np.savez_compressed(
            f"./data/{data_file}",
            alpha=samples.alpha,
            K0=samples.K0,
