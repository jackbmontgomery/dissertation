...
def parallel_nuts_sampling(
    ...
    nuts = blackjax.nuts(log_density, 1e-3, inverse_mass_matrix)

    inference_loop_multiple_chains = pmap(
        _inference_loop, in_axes=(0, None, 0, None), static_broadcasted_argnums=(1, 3)
    )

    initial_states = vmap(nuts.init, in_axes=(0))(initial_parameters)
...
states = parallel_nuts_sampling(keys, n_samples_per_chain, chain_params, log_density)
