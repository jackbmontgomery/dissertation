...
In a typical electrochemical experiment, a device called a
\emph{potentiostat} applies a controlled, time-varying potential to a
\emph{working electrode} immersed in an electrolyte solution and
simultaneously records the resulting current
(\cref{fig:electrochemical-cell}). A \emph{reference electrode} provides
a stable potential against which the working electrode is measured, and a
\emph{counter electrode} completes the circuit. This is depicted in
\cref{fig:electrochemical-cell}.
...
single-electron process and adopt the convention that a
...
electron transfer and reforming~$\mathrm{A}$.
Throughout this process the current $I$ is recorded. The triangular
waveform and the resulting voltammogram are shown in the left and
right panels of \cref{fig:potential-voltammogram}, respectively.

The forward sweep produces
...
consider a planar disc electrode of radius~$r_e$, whose characteristic
...
\begin{equation}
  ...
  \label{eq:current}
\end{equation}
where $A_{\mathrm{el}} = \pi r_e^2$ is the area of the disc electrode
...
concentrations by the bulk concentration
of~$\mathrm{A}$, diffusion coefficients by~$D_A$, lengths by the
electrode radius~$r_e$, and potentials by the thermal voltage
$\mathcal{R}\mathcal{T}/F$. \Cref{tab:normalisation} summarises the
scalings.
...
\begin{table}[ht]
  ...
  \begin{tabular}{ll}
    ...
    \hline
  \end{tabular}
  \caption{Dimensionless normalisation of variables.}
...
\begin{equation}
  ...
  \frac{x}{r_e}, \quad T = \frac{D_A\, t}{r_e^2},
\end{equation}
and the dimensionless rate constant as
\begin{equation}
  K_0 = \frac{k_0\,r_e}{D_A}.
  \label{eq:nd_K0}
\end{equation}
...
Substituting the definitions from \cref{tab:normalisation} into
\cref{eq:diffusion} gives the dimensionless diffusion equation:
\begin{equation}
...
\end{equation}
Substituting the definitions of $\theta$ and $T$ gives
\begin{equation}
  \sigma = \frac{F r_e^2}{\mathcal{R}\mathcal{T} D_A}\, \nu.
  \label{eq:sigma_nu}
\end{equation}

\subsection{Current}

We define the dimensionless flux
\begin{equation}
  J_{X^*,\, j} = -d_j \left(\frac{\partial C_j}{\partial X}\right)_{X^*}.
  \label{eq:nd_flux}
\end{equation}
Substituting into \cref{eq:current} with $A_{\mathrm{el}} = \pi r_e^2$
and using the scalings from \cref{tab:normalisation} gives
\begin{equation}
  I = \pi r_e\, F\, D_A\, c_A^*\, J_{0,A}.
  \label{eq:nondim_current}
\end{equation}
A simulation records $J$ as a function of $\theta$; the physical
voltammogram is recovered by scaling with $c_A^*$, $D_A$, and $r_e$.
A single simulation therefore applies to any combination of these
parameters.
...
discretisation that follows in the next chapter.
