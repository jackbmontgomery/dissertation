...
of the optimisation at two noise levels ($\varsigma = 0.25$ and
$\varsigma = 0.5$). Each algorithm is run from $32$~initial conditions
...
conditions. At $\varsigma = 0.5$ the increased noise flattens the
likelihood surface, making the optimisation harder for both methods;
CMA-ES exhibits wider spread and slower convergence in this regime,
whereas ADAM remains comparatively stable. Importantly, once ADAM reaches
the high-density region it remains there with little fluctuation,
confirming the stability required for a reliable initialisation of the
subsequent MCMC sampling.
...
\begin{figure}[ht]
  ...
  \caption{Log-posterior density during optimisation for ADAM (blue) and
    CMA-ES (orange) at noise levels $\varsigma = 0.25$ (left) and
    $\varsigma = 0.5$ (right). Each algorithm is run from
