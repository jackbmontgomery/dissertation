...
a redox-active film (for example, a self-assembled monolayer of
ferrocene-terminated thiols on gold~\cite{compton2014understanding})
and the voltammetry is performed in a blank electrolyte. The Langmuir
adsorption terms contain products of surface coverages and
...
implicit linearisation. The parameter space grows to nine dimensions.
...
\begin{equation}\label{eq:ads-thetaf-relation}
  ...
  \right).
\end{equation}
This is not a free parameter: $\theta_f^{\mathrm{ads}}$ is
...
the true parameters, confirming that the
approximation~\cref{eq:ads-britz} does not introduce systematic bias
at this noise level.
...
\subsection{Convergence and Sampling Efficiency}\label{sec:ads-ess}
...
cross the threshold for most parameters. $K_0^{\mathrm{sol}}$ is the
exception: both samplers converge almost
immediately, reflecting its well-constrained marginal.
...
\begin{figure}[ht]
  ...
  of the budget to cross $\hat{R} = 1.01$ for most parameters.}
  \label{fig:ads-gr}
\end{figure}
