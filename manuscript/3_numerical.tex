...
We now solve the initial-boundary value problem from the previous
...
Time is discretised uniformly into $m$ steps

\begin{equation}
  m = \frac{T_{\mathrm{max}}}{\Delta T} = \frac{2(\theta_v -
  \theta_i)}{\sigma \Delta T}
  \label{eq:temporal_discritisation}
\end{equation}

Instead of defining $\Delta T$ directly, since this would mean any
change in $\sigma$ would also change the number of
temporal steps we use and thus the accuracy of the simulation. We instead
define the change in
potential per timestep $\Delta \theta = \sigma \Delta T$. This fixes
the number of timesteps per solve independent of the scan
rate~\cite{compton2014understanding}.
...
\end{equation}
The far-field node imposes $C_{n-1}^k = 1$. The resulting
tridiagonal system is solved in $O(n)$ operations by the Thomas algorithm.
...
surface flux. The current is the experimental observable and the sole
target of inference, so we evaluate it with a higher-order three-point formula.
Fitting a quadratic through $(X_0, C_0^k)$, $(X_1, C_1^k)$, $(X_2,
C_2^k)$ and differentiating at $X_0$ gives
\begin{equation}
  J_{0,A}^k
...
\section{Convergence Study}\label{sec:convergence}
...
Euler scheme. We adopt $h_0 = 10^{-6}$ and $\Delta\theta = 0.004$
throughout (\cref{fig:electron-discritisation}), which places the
solution well within the converged regime at acceptable runtime. The
same methodology was applied to the heterogeneous and adsorption
solvers (\cref{ch:appendix-convergence}).

\begin{figure}[ht]
  \centering
  \includegraphics[width=0.95\textwidth]{figures/3-discretisation.png}
  \caption[Convergence of the electron-transfer solver]{Convergence of
    the electron-transfer solver. Relative $L^2$ error of the simulated
    current against a reference solution ($h_0 = 10^{-10}$,
    $\Delta\theta = 10^{-5}$). Left: spatial refinement at four fixed
    $\Delta\theta$; the error plateaus for $h_0 \lesssim 10^{-6}$ where
    the temporal error dominates. Right: temporal refinement at $h_0 =
    10^{-6}$; the measured slope matches the first-order rate expected
    for backward Euler. Dotted lines mark the adopted values $h_0 =
  10^{-6}$ and $\Delta\theta = 4 \times 10^{-3}$.}
  \label{fig:electron-discritisation}
\end{figure}
...
$\theta_f$ can partially compensate -- the parameters are correlated and
...
regime and negligible differences in the reversible regime motivates a
...
\begin{figure}[ht]
  \begin{center}
    \includegraphics[width=0.9\textwidth]{figures/3-alpha-K0-effect-reversible.png}
  \end{center}
