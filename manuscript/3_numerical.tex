...
T_{\max}/m$, with $T_{\max} = 2|\theta_v - \theta_i|/\sigma$ for a
cyclic voltammogram sweeping from $\theta_i$ to $\theta_v$ and back.
...
X_{i+1} - X_i$, $\Delta X_- = X_i - X_{i-1}$.
...
heterogeneous (\cref{ch:heterogeneous}) and adsorption
(\cref{ch:adsorption}) solvers, with comparable results.
...
\end{equation}
When $\Lambda \gg 1$ the reaction is reversible (electron transfer is
...
Both $\alpha$ and $K_0$ become practically non-identifiable once
electron transfer is no longer rate-limiting. The combination of
parameter correlations in the quasi-reversible regime and
non-identifiability in the reversible regime motivates a
...
\begin{figure}[ht]
  ...
  \caption[Non-identifiability of the transfer coefficient and rate
    constant in the
  reversible regime]{Left: effect of $\alpha$ in the reversible regime
